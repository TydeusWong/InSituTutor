...
- If step intent (from prompt/focus_points/common_mistakes/transcript) contains hand-action verbs such as remove/open/close/unscrew/screw/twist/press/push/pull/grab/pick/place/hold, you must include `mediapipe-hand-landmarker` (not DINO-only).
- Minimal-model override for hand:
  - If this step can be decided by final object state/relations only (e.g., place/move/align/stack result), do NOT include `mediapipe-hand-landmarker`; use DINO-only.
...
- For `grounding-dino`, `detect_targets` must be chosen only from payload field `allowed_dino_detect_targets`, whose source is `scene_entity_catalog.entity_names` from `scene_entities_v1.json`.
- For `grounding-dino`, `detect_targets` must reuse exact strings from `scene_entity_catalog.entity_names` as-is; do not rewrite, translate, abbreviate, normalize, or replace with synonyms.
- Once you decide to use `grounding-dino`, you must select at least one target in `detect_targets`.
- `grounding-dino` target names must be strictly character-exact with `scene_entity_catalog.entity_names`; never use a name outside the list.
